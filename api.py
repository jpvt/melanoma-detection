...
app = Flask(__name__)
UPLOADE_FOLDER = "static/data_storage"
DEVICE = "cuda"
MODEL = None
MODEL_PATH = "data_storage/model_0.bin"
...
def upload_predict():
    ...
    if request.method == "POST":
        ...
        if image_file:
            image_path = os.path.join(UPLOADE_FOLDER, image_file.filename)
            image_file.save(image_path)
            pred = predict(image_path, MODEL)[0]
            print(pred)
            return render_template('index.html', prediction=round(pred, 3), image_loc = image_path)

    return render_template('index.html', prediction=0, image_loc = image_path)
